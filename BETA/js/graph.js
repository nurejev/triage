...
(function () {
  ...
  // Tenant-wide Unified Audit Log query (no user filter) - the BETA analyzes
  // the whole tenant window, not one account.
  async function ualQueryTenant(startIso, endIso, onStatus) {
    const base = A.graphV1 + "/security/auditLog/queries";
    const job = await gfetch(base, {
      method: "POST",
      body: JSON.stringify({
        "@odata.type": "#microsoft.graph.security.auditLogQuery",
        displayName: "BETA tenant " + new Date().toISOString(),
        filterStartDateTime: startIso,
        filterEndDateTime: endIso
      })
    });
    const id = job.id;
    let status = job.status || "notStarted", waited = 0;
    while (status !== "succeeded") {
      if (status === "failed" || status === "cancelled") throw new Error("Audit log query " + status);
      if (waited > 600000) throw new Error("Audit log query timed out (10 min)");
      const wait = waited < 60000 ? 5000 : 15000;
    ...
    return gall(base + "/" + id + "/records?$top=1000", 120);
  }
  ...
  window.TriageGraph = {
    ...
    gfetch: gfetch, gall: gall, gwrite: gwrite, ualQuery: ualQuery, ualQueryTenant: ualQueryTenant
  };
})();
